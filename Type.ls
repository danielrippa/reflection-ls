  ...
  do ->
    ...
    { type-descriptor } = dependency 'reflection.TypeDescriptor'
    { each-array-item: each-item, find-array-item: find-item, array-size, drop-array-items, is-array, map-array-items: map } = dependency 'unsafe.Array'
    { is-a } = dependency 'reflection.TypeName'
    { trimmed-string, string-contains } = dependency 'unsafe.String'
    { camel-case } = dependency 'unsafe.StringCase'
    { type-error } = dependency 'reflection.TypeError'
    { function-parameter-names, is-function } = dependency 'unsafe.Function'
    { typed-value-as-string, value-as-string } = dependency 'reflection.Value'
    { trimmed-string } = dependency 'unsafe.Whitespace'
    { is-object, object-member-names } = dependency 'unsafe.Object'
